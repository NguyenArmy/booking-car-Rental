  ...
  return (
    ...
    className={`flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 text-gray-600 border-b border-borderColor relative transition-all ${location.pathname ==="/" && "bg-light" }`}>
        ...
        ${open? "max-sm:translate-x-0" : "max-sm:translate-x-full"}` }>
            ...
            <input type="text" className="py-1.5 w-full bg-transparent outline-none placeholder-gray-500 placeholder:text-sm" placeholder="Search..." />
            ...
            <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
                <button type="button" onClick={() => isOwner ? navigate('/owner') : changeRole()} className="cursor-pointer">{isOwner ? 'Dashboard' : 'List cars'}</button>
                <button 
                  type="button"
                  onClick={() => {
